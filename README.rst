...
Please send an email to ``yingzu AT sjtu.edu.cn`` if you have any questions.
    ...
    *Version 0.5.3* is now pure python, and please go to `JAVELIN <https://github.com/nye17/javelin>`_ for the previous version that is compatible with Python 3 but requires `f2py`.
...
You can install JavelinTNG by the standard Python package installation procedure::
...
After installing JavelinTNG, navigate into the ``examples`` directory::
