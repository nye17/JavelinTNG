...
def generateTrueLC(covfunc="kepler2_exp"):
    r"""Generate one continuum realization and two top-hat line responses.

    The photometric-band mock curves are built from the *same* continuum and
    line realizations: YelmBand = continuum + Yelm and YelmZingBand =
    Yelm + Zing.  This preserves the covariance structure assumed by Pmap and
    DPmap.
    """
    jmin = np.max(lags)
    ...
    # Continuum, cropped to the epoch range common to all lagged curves.
    imin = np.searchsorted(jdense, jmin)
    imax = np.searchsorted(jdense, jmax)
    j_common = jdense[imin:imax]
    e_common = edense[imin:imax]
    cont = sdense[imin:imax] + lcmeans[0]

    line_fluxes = []
    for i in range(1, 3):
        jl, sl = generateLine(jdense, sdense, lags[i], wids[i], scales[i],
                              mc_mean=0.0, ml_mean=0.0)
        # ``generateLine`` labels the response at shifted epochs jc + lag.
        # Evaluate that response on the common output grid so all components
        # are combined at identical physical epochs, as in the historical
        # joint PredictSpear realization.  j_common starts at max(lag), so no
        # extrapolation is required.
        sl_common = np.interp(j_common, jl, sl) + lcmeans[i]
        line_fluxes.append(sl_common)

    yelm, zing = line_fluxes
    zylist = [
        [j_common, cont, e_common],
        [j_common, yelm, e_common],
        [j_common, zing, e_common],
        [j_common, cont + yelm, e_common],
        [j_common, yelm + zing, e_common],
    ]
    return LightCurve(zylist, names=names)

def generateTrueLC2(covfunc="drw"):
    r"""Generate the dense DRW/RMap demonstration light curves efficiently.

    Historically this helper constructed a single joint covariance matrix for
    three 2000-point light curves and factorized the resulting 6000 x 6000
    matrix.  That was tolerable with the original compiled Fortran/LAPACK
    stack, but is unnecessarily expensive for the demonstration: the line
    curves are deterministic top-hat responses to the same DRW continuum.

    We therefore generate one dense continuum realization and construct the
    two response curves by convolution, which is the same construction used by
    :func:`generateTrueLC`.  This avoids the O(N^3) 6000-point Cholesky while
    preserving the intended DRW + top-hat mock model.
    """
    if covfunc != "drw":
        raise RuntimeError(
            "generateTrueLC2 only supports the DRW covariance; "
            "use generateTrueLC for other covariance functions"
        )
    return generateTrueLC(covfunc="drw")
...
def demo(mode, covfunc="drw") :
    ...
    if True :
        ...
        try :
            ...
            threads = 1
        if threads > 1 :
            print(("use MCMC parallel workers: %d"%threads))
        else :
    ...
    # ``run`` is intended as an integration/demo run, not a production chain.
    # Set JAVELIN_DEMO_FULL=1 to recover the historical 100/100/100 settings.
    import os
    if mode == "run" and os.environ.get("JAVELIN_DEMO_FULL", "0") not in {"1", "true", "TRUE", "yes", "YES"}:
        demo_nwalkers, demo_nburn, demo_nchain = 24, 10, 10
        print("demo MCMC settings: 24 walkers, 10 burn-in steps, 10 sampling steps")
        print("set JAVELIN_DEMO_FULL=1 for the historical 100/100/100 demo chains")
    else:
        demo_nwalkers, demo_nburn, demo_nchain = 100, 100, 100
    ...
    # fit continuum
    conthpd = fitCon(confile, confchain, names=names[0:1], threads=threads, set_plot=set_plot, nwalkers=demo_nwalkers, nburn=demo_nburn, nchain=demo_nchain, mode=mode)
    if conthpd is None:
    ...
    # fit tophat
    tophpd = fitLag(topfile, topfchain, conthpd, names=names[0:2], threads=threads, set_plot=set_plot, nwalkers=demo_nwalkers, nburn=demo_nburn, nchain=demo_nchain, mode=mode)
    if tophpd is None:
    ...
    # fit douhat
    douhpd = fitLag(doufile, doufchain, conthpd, names=names[0:3], threads=threads, nwalkers=demo_nwalkers, nburn=demo_nburn, nchain=demo_nchain,set_plot=set_plot, mode=mode)
    if douhpd is None:
    ...
    # fit pmap
    phohpd = fitPmap(phofile, phofchain, conthpd, names=[names[0], names[3]], lagrange=[-50, 300], lagbinsize=0.2, threads=threads, nwalkers=demo_nwalkers, nburn=demo_nburn, nchain=demo_nchain, set_plot=set_plot, mode=mode)
    if phohpd is None:
    ...
    # fit dpmap
    dphhpd = fitDPmap(dphfile, dphfchain, conthpd, names=[names[0], names[4]], lagrange=[50, 300], lagbinsize=0.2, threads=threads, nwalkers=demo_nwalkers, nburn=demo_nburn, nchain=demo_nchain, set_plot=set_plot, mode=mode)
    if dphhpd is None:
