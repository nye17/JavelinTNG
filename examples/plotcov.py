...
def show_nurange(cax, numin, numax, vmin=None, vmax=None, covfunc="pow_exp", set_color=True):
    ...
    cb.set_ticklabels(["$"+str(numin)+"$", r"$\longrightarrow$", "$"+str(numax)+"$"])
    cb.set_label(nuname)
...
def show_drw(ax, xtuple=None, set_color="color"):
    ...
    plotcov(ax, covfunc=covfunc, color="k", ls="--", lw=2.5, scale=1.0,
            transparency=0.5, xtuple=xtuple, label=r"$\mathrm{DRW}$")
...
def covdemo(set_color=True):
    ...
    for ax, id in zip(axes, aids):
        ...
        ax.annotate(id, xy=(0.05,0.05),
            ...
            fontsize=14,)
    ax3.set_xlabel(r"$\Delta t/\tau$")
    ax4.set_xlabel(r"$\Delta t/\tau$")
    ax1.set_ylabel(r"$\mathrm{covariance}$")
    ax3.set_ylabel(r"$\mathrm{covariance}$")
    ax1.set_xticklabels([])
    ax2.set_xticklabels([])
    ax2.set_yticklabels([])
    ax4.set_yticklabels([])
