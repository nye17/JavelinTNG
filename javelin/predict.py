...
def _scalarize(value):
    """Return a Python float from a scalar or a size-1 ndarray/matrix."""
    arr = np.asarray(value)
    if arr.size != 1:
        raise ValueError(f"Expected scalar covariance value, got shape {arr.shape}")
...
class PredictSignal(object):
    ...
    def __init__(self, zydata=None, lcmean=0.0, covfunc="drw", rank="Full", **covparams):
        ...
        elif (rank == "NearlyFull") or rank == "NearlyFullRankCovariance" :
...
class PredictRmap(object):
    ...
    def _fastpredict(self, jw, iw, set_threading=False) :
        ...
        for i, (jwant, iwant) in enumerate(zip(jw, iw)):
            ...
            if set_threading :
                vw[i] = _scalarize(spear_threading(jwant, jwant, iwant, iwant, **self.covparams))
            else :
                vw[i] = _scalarize(spear(jwant, jwant, iwant, iwant, **self.covparams))
            mw[i] = _scalarize(np.dot(covar, self.cplusninvdoty))
            vw[i] = vw[i] - _scalarize(np.dot(covar, cplusninvdotcovar))
        return(mw, vw)
...
class PredictPmap(object):
    ...
    def _fastpredict(self, jw, iw, set_threading=False) :
        ...
        for i, (jwant, iwant) in enumerate(zip(jw, iw)):
            ...
            cplusninvdotcovar = chosolve_from_tri(self.U, covar.T, nugget=None, inplace=False)
            if set_threading :
                vw[i] = _scalarize(spear_threading(jwant, jwant, iwant, iwant, set_pmap=True, **self.covparams))
            else :
                vw[i] = _scalarize(spear(jwant, jwant, iwant, iwant, set_pmap=True, **self.covparams))
            mw[i] = _scalarize(np.dot(covar, self.cplusninvdoty))
            vw[i] = vw[i] - _scalarize(np.dot(covar, cplusninvdotcovar))
        return(mw, vw)
...
class PredictSPmap(object):
    ...
    def _fastpredict(self, jw, iw, set_threading=False) :
        ...
        for i, (jwant, iwant) in enumerate(zip(jw, iw)):
            ...
            cplusninvdotcovar = chosolve_from_tri(self.U, covar.T, nugget=None, inplace=False)
            if set_threading :
                vw[i] = _scalarize(spear_threading(jwant, jwant, iwant+1, iwant+1, set_pmap=True, **self.covparams))
            else :
                vw[i] = _scalarize(spear(jwant, jwant, iwant+1, iwant+1, set_pmap=True, **self.covparams))
            mw[i] = _scalarize(np.dot(covar, self.cplusninvdoty))
            vw[i] = vw[i] - _scalarize(np.dot(covar, cplusninvdotcovar))
        return(mw, vw)
...
class PredictSCmap(object):
    ...
    def _fastpredict(self, jw, iw, set_threading=False) :
        ...
        for i, (jwant, iwant) in enumerate(zip(jw, iw)):
            ...
            cplusninvdotcovar = chosolve_from_tri(self.U, covar.T, nugget=None, inplace=False)
            if set_threading :
                vw[i] = _scalarize(spear_threading(jwant, jwant, iwant+1, iwant+1, **self.covparams))
            else :
                vw[i] = _scalarize(spear(jwant, jwant, iwant+1, iwant+1, **self.covparams))
            mw[i] = _scalarize(np.dot(covar, self.cplusninvdoty))
            vw[i] = vw[i] - _scalarize(np.dot(covar, cplusninvdotcovar))
        return(mw, vw)
...
class PredictDPmap(object):
    ...
    def _fastpredict(self, jw, iw, set_threading=False) :
        ...
        for i, (jwant, iwant) in enumerate(zip(jw, iw)):
            ...
            cplusninvdotcovar = chosolve_from_tri(self.U, covar.T, nugget=None, inplace=False)
            if set_threading :
                vw[i] = _scalarize(spear_threading(jwant, jwant, iwant, iwant, set_dpmap=True, **self.covparams))
            else :
                vw[i] = _scalarize(spear(jwant, jwant, iwant, iwant, set_dpmap=True, **self.covparams))
            mw[i] = _scalarize(np.dot(covar, self.cplusninvdoty))
            vw[i] = vw[i] - _scalarize(np.dot(covar, cplusninvdotcovar))
        return(mw, vw)
