...
class Covariance(object):
    ...
    def continue_cholesky(self, x, x_old, chol_dict_old, apply_pivot = True, observed=True, nugget=None, regularize=True, assume_full_rank = False, rank_limit=0):
        ...
        U = zeros((m_new_max + m_old, N_old + N_new), dtype=float, order='F')
    ...
    def _unobs_reg(self, M):
        ...
        return trisolve(self.Uo, M.dev.T, uplo='U', transa='T').T
    ...
    def _obs_reg(self, M, dev_new, m_old):
        ...
        reg_mat_new += trisolve(self.Uo[m_old:, m_old:], dev_new.T, uplo='U', transa='T').T
        return vstack((M.reg_mat, reg_mat_new))
