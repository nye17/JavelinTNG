...
_WORKER_LNPROBFN = None

def _init_lnprob_worker(fn):
    global _WORKER_LNPROBFN
    _WORKER_LNPROBFN = fn
    try:
...
def _worker_lnprob(p):
    return _WORKER_LNPROBFN(p)
...
class EnsembleSampler(Sampler):
    ...
    **Warning**: The :attr:`chain` member of this object has the shape:
    ...
    def __init__(self, nwalkers, dim, lnpostfn, a=2.0, args=[], kwargs={},
        ...
        self._persistent_worker_pool = False
        ...
        if self.threads > 1 and self.pool is None:
            ...
            self.pool = InterruptiblePool(
                self.threads, initializer=_init_lnprob_worker,
                initargs=(self.lnprobfn,), context=ctx
            )
            self._persistent_worker_pool = True
    ...
    def _get_lnprob(self, pos=None):
        ...
        * ``blob`` - The list of meta data returned by the ``lnpostfn`` at
        ...
        func = _worker_lnprob if (self.pool is not None and self._persistent_worker_pool) else self.lnprobfn
        results = list(M(func, [p[i] for i in range(len(p))]))
